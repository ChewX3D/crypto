...
## Current State In AGENTS And CLAUDE.md (MANDATORY)

`AGENTS.md` is the canonical current-state snapshot of the system. `CLAUDE.md` is a synchronized copy consumed by Claude Code agents.
...
- `AGENTS.md` and `CLAUDE.md` must always be identical in content, except for their first-line file heading (`# AGENTS.md` vs `# CLAUDE.md`)
- when modifying `AGENTS.md`, apply the same changes to `CLAUDE.md` in the same change set
- when modifying `CLAUDE.md`, apply the same changes to `AGENTS.md` in the same change set
...
- current-state sections in `AGENTS.md` and `CLAUDE.md` (always together — see sync rule)
